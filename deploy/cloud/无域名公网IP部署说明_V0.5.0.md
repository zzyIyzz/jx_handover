...
- 云端应用内部端口固定为 `1215`，公网只使用 HTTPS 443。
...
2. 阿里云安全组不开放 TCP 1215；Docker 只发布 `127.0.0.1:1215`。
3. TCP 80 必须长期允许 ACME HTTP-01 校验，否则约 6 天后证书会失效。
4. TCP 443 第一轮只允许部署管理员当前网络的公网出口 IPv4；共享口令不能直接暴露给任意公网来源。
...
| 443 | 管理员或单位出口公网 IPv4/32 | 交接班 HTTPS |
| 1215 | 不创建规则 | 仅 ECS 本机 Nginx 使用 |
...
ss -lntp | grep -E ':(80|443|1215)\b' || true
```
...
- `/www` 有足够空间；
- 1215 空闲，或者已经由本系统当前容器占用；
- 1215 被其他程序占用时，部署脚本会停止但不会结束该程序。
...
JX_PUBLIC_URL=https://你的真实公网IPv4
...
只能看到宿主机 `127.0.0.1:1215`；不能看到 `0.0.0.0:1215`。

本机健康检查需要正式 Host：

```bash
curl -H 'Host: 你的真实公网IPv4' http://127.0.0.1:1215/api/health
```

应返回 `status: ok`、`version: 0.5.0`、`mode: cloud` 和 `port: 1215`。
...
- 备份宝塔原站点配置；
- 写入证书路径、ACME 续签例外、IP 白名单和 `127.0.0.1:1215` 反向代理；
- 执行 Nginx 配置检查；
- 检查失败时自动恢复原配置；
- 检查成功后重载 Nginx。
...
3. 修改 `.env` 的 `JX_PUBLIC_URL` 和 `JX_TRUSTED_HOSTS`；
4. 重建/重启应用；
5. Nginx 仍反代 `127.0.0.1:1215`；
