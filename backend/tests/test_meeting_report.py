...
class MeetingReportTests(unittest.TestCase):
    ...
    def _batch_with_item(
        ...
        batch = HandoverBatch(start_date="2026-09-01", end_date="2026-09-10",
                              handover_date="2026-09-10", status=batch_status)
        self.db.add(batch)
        self.db.flush()
    ...
    def test_monthly_meeting_excludes_unpublished_drafts(self):
        self._batch_with_item(
            "completed",
            "urgent",
            "important",
            "尚未发布的草稿",
    ...
    def test_yearly_overview_with_memory(self):
        ...
        # y4 只有计划、没有执行记录 → 未开展
        self.assertEqual(by_id["y4"]["state"], "pending")

    def test_periodic_memory_endpoint(self):
        self._yearly_plan("y1", "2025-01", status="completed", note="历史备注")
        self._yearly_plan("y1", "2026-01", status="pending", note="本年草稿")
        result = meeting_report.periodic_memory(
            self.db,
            ["y1", "y99", "bad"],
            before_year=2026,
        )
