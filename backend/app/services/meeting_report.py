...
def _batches_in_month(db: Session, month_start: str, month_end: str
    ...
    return (
        ...
        .filter(HandoverBatch.start_date <= month_end,
...
def periodic_memory(db: Session, library_ids: list[str]) -> dict:
    """6.3 录入时的“上次记录”：每个模板项最近一次填写内容。"""
    ids = [lid for lid in library_ids if lid in periodic.LIBRARY_BY_ID][:200]
    if not ids:
        return {"memory": {}}
    plans = (db.query(MonthlyPlanItem)
             .filter(MonthlyPlanItem.library_id.in_(ids)).all())
